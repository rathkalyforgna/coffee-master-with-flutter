...
class _OrderPageState extends State<OrderPage> {
  ...
  Widget build(BuildContext context) {
    if (widget.dataManager.carts.isEmpty) {
      return const Center(child: Text("You have no order."));
    } else {
      return ListView.builder(
        itemCount: widget.dataManager.carts.length,
        itemBuilder: ((context, index) {
          var item = widget.dataManager.carts[index];
          return OrderItem(
              item: item,
              onRemove: (product) {
                setState(() {
